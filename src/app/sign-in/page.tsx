...
const SignIn = () => {
  ...
  const redirectedFromCart = searchParams?.get("cart") === "false";
  const [accountCreated, setAccountCreated] = useState(userCreated);
  const [fromCart, setFromCart] = useState(redirectedFromCart);
  ...
  }, [handleError, accountCreated, fromCart]);
      ...
      <div className="min-h-[90vh] flex items-center justify-center px-4 py-16 bg-gray-50">
        <div className="w-full max-w-md bg-white shadow-lg rounded-xl p-6 sm:p-8">
          ...
          <form onSubmit={handleSubmit} className="space-y-5">
            ...
            <button
              type="submit"
              className="w-full py-3 px-6 bg-indigo-600 text-white rounded-lg hover:bg-indigo-700 transition ease-in-out duration-500 font-semibold"
            >
              Sign In
            </button>

            <p className="text-center text-sm text-gray-500 hover:underline hover:text-gray-800">
              <Link
                href="/create-account"
            ...
            <Link
              href="/"
              className="block text-sm text-center text-gray-500 hover:underline hover:text-gray-800"
            >
              ← Back to Home
            </Link>
          </form>
